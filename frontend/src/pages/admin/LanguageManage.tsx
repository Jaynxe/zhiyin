...
import type {Language, PaginationState, QueryParams} from "@/types";
import {
    Button,
    Card,
    Form,
...
function LanguageManage() {
    ...
    const handleModalOk = async () => {
        try {
            ...
            if (isEdit) {
                if (!currentId) return;
                await apiClient.updateLanguage(currentId, formData);
                messageApi.success("更新成功");
            } else {
                await apiClient.createLanguage(formData);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await apiClient.getLanguageList(queryParams);
                ...
                if (isAxiosError(err)) {
                    messageApi.error(err.response?.data?.msg || "获取语言失败，请重试");
                } else {
                    messageApi.error("获取语言时发生未知错误");
                }
            } finally {
                setLoading(false);
            }
    ...
    const handelDelete = async (ids: React.Key[]) => {
        try {
            const res = await apiClient.deleteLanguage(ids);
            ...
            <Card
                ...
                extra={
                    <Space>
                        ...
                        <Popconfirm
                            title="批量删除日志"
                            description={`你确定要删除这${selectedRowKeys.length}门语言吗?`}
